...
data_type = {'test','pretrain','trainval'};
mix_info_folder='./mixed_audio/same'
wsj0root = ''; % YOUR_PATH/, the folder containing wsj0/
%here is none, since file contains full_file_path

output_mix_file='./mixed_audio/same/mix_info'
output_dir16k='/CDShare3/LRS3_process/Simulated/mixaudio_16k_s';
...
if ~exist(output_mix_file,'dir')
    mkdir(output_mix_file);
end
...
for i_mm = 1:length(min_max)
    for i_type = 1:length(data_type)
        ...
        TaskFile = [mix_info_folder '/' 'mix_2_spk_' data_type{i_type} '.txt'];
        fid=fopen(TaskFile,'r');
        C=textscan(fid,'%s %f %s %f');
        
        Source1File = [output_mix_file '/' 'mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_1'];
        Source2File = [output_mix_file '/' 'mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_2'];
        MixFile     = [output_mix_file '/' 'mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_mix'];
        ...
        for i = 1:num_files
            ...
            inwav1_dir_end = strsplit(inwav1_dir,'/')
            inwav1_dir_end = inwav1_dir_end{end}

            inwav2_dir_end = strsplit(inwav2_dir,'/')
            inwav2_dir_end = inwav2_dir_end{end}
            ...
            inwav1_snr = C{2}(i);
            inwav2_snr = C{4}(i);
            mix_name = [inwav1_dir_end,'_',invwav1_name,'_',num2str(inwav1_snr),'_',inwav2_dir_end,'_',invwav2_name,'_',num2str(inwav2_snr)];
            fprintf(fid_m,'%s\n',mix_name);
            s1_name = [mix_name,'+',inwav1_dir_end,'_',invwav1_name]
            s2_name = [mix_name,'+',inwav2_dir_end,'_',invwav2_name]
            ...
            if useaudioread                          
                ...
                audiowrite([output_dir16k '/' min_max{i_mm} '/' data_type{i_type} '/s1/' s1_name '.wav'],s1_16k,fs);
                % audiowrite([output_dir8k '/' min_max{i_mm} '/' data_type{i_type} '/s2/' mix_name '.wav'],s2_8k,fs8k);
                audiowrite([output_dir16k '/' min_max{i_mm} '/' data_type{i_type} '/s2/' s2_name '.wav'],s2_16k,fs);
